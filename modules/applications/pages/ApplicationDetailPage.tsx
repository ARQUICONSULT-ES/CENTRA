    ...
    <div className="space-y-3 sm:space-y-4 md:space-y-6">
            ...
            <div className="p-3 sm:p-4 md:p-5 space-y-3 sm:space-y-4">
              ...
              {application.latestPrereleaseVersion && (
                <div className="flex items-start gap-2 sm:gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mb-0.5">Última versión (Pre-release)</p>
        ...
        <div className="px-3 sm:px-5 py-2 sm:py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => {
                const event = new CustomEvent('openAddEnvironmentModal');
                window.dispatchEvent(event);
              }}
              disabled={!owner || !repo}
              className="px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-1 sm:gap-1.5 w-full sm:w-auto justify-center"
            >
              <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
              <span className="hidden sm:inline">Añadir Entornos</span>
      ...
      {owner && repo && (
        <PublishToEnvironmentModal
          ...
          latestVersion={application.latestReleaseVersion}
          latestDate={application.latestReleaseDate}
          prereleaseVersion={application.latestPrereleaseVersion}
          prereleaseDate={application.latestPrereleaseDate}
        />
      )}
    </div>
  );
